...
function updateTeamChampionship() {
    ...
    if (teamsArray.length === 0) {
        ...
        leaderboardContainer.innerHTML = `
            <div style="text-align:center; padding:3rem; color:#64756A; font-style:italic; font-weight:500; width: 100%;">
        ...
        if (mobLeaderboardContainer) {
            mobLeaderboardContainer.innerHTML = `
                <div style="text-align:center; padding:2rem 1rem; color:#64756A; font-style:italic; font-weight:600; width: 100%;">
                    No team points have been published yet.
                </div>
            `;
        }
    ...
    if (mobileLeaderboardContainer) {
        const teamColors = [
            'linear-gradient(90deg, #fbbf24, #F59E0B)', // Gold/Orange (1st)
            'linear-gradient(90deg, #64756A, #64756A)', // Silver/Slate (2nd)
            'linear-gradient(90deg, #fdba74, #c2410c)', // Bronze/Red (3rd)
            'linear-gradient(90deg, #3b82f6, #1d4ed8)', // Blue
            'linear-gradient(90deg, #059669, #047857)', // Emerald
            'linear-gradient(90deg, #059669, #065F46)', // Violet
...
function updateCategoryPerformance() {
    ...
    if (!data || data.length === 0) {
        const emptyHTML = `
            <div style="text-align:center; padding:2.5rem; color:#64756A; font-style:italic; font-weight:500; width:100%;">
...
function getMiniPosterHTML(r, bgId, templateId, resultNumber, madrasaName) {
    ...
    if (templateId === 2) {
        ...
        const bentoGridHTML = hasWinners ? `
            <div style="display: flex; flex-direction: column; gap: 4px; width: 100%; box-sizing: border-box;">
                ...
                <div style="display: flex; gap: 4px; width: 100%;">
                    <!-- 2nd Place Card -->
                    <div style="background: rgba(255, 255, 255, 0.05); border: 0.5px solid rgba(255, 255, 255, 0.1); border-top: 1.5px solid #D8E8DE; border-radius: 8px; padding: 4px 6px; width: calc(50% - 2px); display: flex; flex-direction: column; justify-content: space-between; position: relative; min-height: 28px; box-sizing: border-box;">
                    ...
                    <!-- 3rd Place Card -->
                    <div style="background: rgba(255, 255, 255, 0.05); border: 0.5px solid rgba(255, 255, 255, 0.1); border-top: 1.5px solid #F59E0B; border-radius: 8px; padding: 4px 6px; width: calc(50% - 2px); display: flex; flex-direction: column; justify-content: space-between; position: relative; min-height: 28px; box-sizing: border-box;">
        ...
        return `
            ...
            <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(23, 37, 27, 0.32); backdrop-filter: blur(2px); -webkit-backdrop-filter: blur(2px); z-index: 1;"></div>
        ...
        return `
            <!-- Mini Dark Overlay & Blur -->
            <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(23, 37, 27, 0.35); backdrop-filter: blur(2px); -webkit-backdrop-filter: blur(2px); z-index: 1;"></div>
        ...
        return `
            <!-- Mini Dark Overlay & Blur -->
            <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(23, 37, 27, 0.50); backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); z-index: 1;"></div>
                ...
                <div class="t2-bento-row">
                    ...
                    <div class="t2-card-bento t2-card-2nd">
                        ...
                        <div class="t2-details" style="margin-top: auto; z-index: 2;">
                            <span style="color: #D8E8DE; font-size: 10px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 4px; display: block;">${w2 ? ordinalLabel(w2.rank) : '2ND'} STANDING</span>
...
function renderError(title, msg) {
    ...
    const emptyHTML = `
        <div class="empty-state" style="text-align: center; padding: 3rem 1.5rem; max-width: 500px; margin: 30px auto; background: rgba(255,255,255,0.85); border-radius: 28px; border: 1px solid rgba(220,38,38,0.15); box-shadow: 0 15px 35px rgba(0,0,0,0.02);">
            ...
            <h2 style="color:#EF4444; margin-bottom:0.5rem; font-family:'Playfair Display', serif; font-weight:700;">${escapeHTML(title)}</h2>
            <p style="color:#64756A; font-weight:500; margin-bottom: 1.5rem;">${escapeHTML(msg)}</p>
            <button type="button" class="btn-action-secondary btn-back-leaderboard" style="width: 100%; padding: 14px; border-radius: 18px; font-weight: 800; font-size: 15px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px;">🏆 View Live Standings</button>
        </div>
    `;
...
async function generatePosterCanvas(r) {
    ...
    const overlayColor = (templateId === 3 || templateId === 4) ? 'rgba(23, 37, 27, 0.35)' : 'rgba(23, 37, 27, 0.32)';
