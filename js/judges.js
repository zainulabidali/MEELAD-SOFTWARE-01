...
function renderJudgesGrid() {
    ...
    filtered.forEach(j => {
        ...
        const statusBadge = isDisabled 
            ...
            : '<span class="badge" style="font-size:0.68rem; background:#ECFDF5; color:#047857; border:1px solid #bbf7d0;">Active</span>';
        ...
        card.innerHTML = `
            ...
            <div class="card-body" style="display:flex; flex-direction:column; gap:0.4rem;">
                <p style="font-size:0.85rem; color:#64756A; margin:0;">
                    📱 <strong>Phone Number:</strong> ${window.escapeHTML(j.mobile)}
                </p>
                <p style="font-size:0.85rem; color:#64756A; margin:0;">
                    📚 <strong>Assigned Programs:</strong> ${comps.length}
                </p>
                ${j.notes ? `<p style="font-size:0.8rem; color:#64756A; margin:0; margin-top:0.2rem;">✏️ <em>${window.escapeHTML(j.notes)}</em></p>` : ''}
            </div>
    ...
    modalBody.innerHTML = `
        <form id="judgeModalForm" autocomplete="off" style="display:flex; flex-direction:column; gap:1rem;">
            ...
            <div class="form-group">
                ...
                <div style="font-size:0.7rem; color:#64756A; margin-top:0.25rem;">Must be a valid digit format between 7 and 15 numbers. Spaces/dashes will be cleaned automatically.</div>
            </div>
            <div class="form-group">
...
async function triggerDeleteJudge(judge) {
    ...
    modalBody.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:1.25rem; padding:0.5rem;">
            ...
            <p style="font-size:0.8rem; color:#64756A; text-align:center; line-height:1.5; margin:0;">
...
async function showViewPopup(judge) {
    ...
    modalBody.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:1.25rem; padding:0.25rem;">
            ...
            <div style="display:flex; align-items:center; gap:1rem; background:#F5FAF7; border:1px solid #D8E8DE; border-radius:12px; padding:1rem;">
                <div style="width:48px; height:48px; background:#064E3B; color:white; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.35rem; font-weight:700; flex-shrink:0;">
                    ${window.escapeHTML((judge.name || '?').slice(0, 1).toUpperCase())}
                </div>
                <div style="flex-grow:1; min-width:0;">
                    <h3 style="margin:0; font-size:1.05rem; font-weight:800; color:#17251B; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                        ${window.escapeHTML(judge.name)}
                    </h3>
                    <div style="font-size:0.8rem; color:#64756A; margin-top:0.15rem; display:flex; gap:0.5rem; flex-wrap:wrap; align-items:center;">
                        <span>📱 ${window.escapeHTML(judge.mobile)}</span>
                        <span>•</span>
                        <span class="badge" style="padding:0.1rem 0.4rem; font-size:0.65rem; text-transform:capitalize; background:${judge.status === 'disabled' ? '#fff1f2; color:#be123c; border:1px solid #fecdd3;' : '#ECFDF5; color:#047857; border:1px solid #bbf7d0;'}">
                ...
                <div style="max-height:280px; overflow-y:auto; display:flex; flex-direction:column; gap:0.6rem; padding-right:0.25rem;">
                    ${assignedList.length > 0 
                        ? assignedList.map(item => `
                            <div style="font-size:0.85rem; color:#17251B; padding:0.65rem 0.85rem; background:#ffffff; border-radius:8px; border:1px solid #D8E8DE; border-left:4px solid #064E3B; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                <div style="font-weight:700; color:#17251B; font-size:0.9rem;">${item.index}. ${window.escapeHTML(item.name)}</div>
                                <div style="margin-left: 1.1rem; margin-top: 0.25rem; font-size: 0.78rem; color: #64756A; display: flex; flex-direction: column; gap: 0.15rem;">
                                    <div><strong>Type:</strong> ${item.type}</div>
                                    ${item.category && item.category !== 'General' ? `<div><strong>Category:</strong> ${window.escapeHTML(item.category)}</div>` : ''}
                                </div>
                            </div>
                        `).join('') 
                        : '<div style="font-size:0.8rem; color:#64756A; text-align:center; padding:1.5rem 0;">No active competition sheets</div>'
                    }
                </div>
            </div>
