...
const TEAM_COLORS_PALETTE = [
    'linear-gradient(90deg, #F59E0B, #F59E0B)', // Gold/Amber
    'linear-gradient(90deg, #3B82F6, #1D4ED8)', // Blue
    'linear-gradient(90deg, #059669, #047857)', // Emerald
    'linear-gradient(90deg, #059669, #065F46)', // Purple
...
function renderTeamChampionship() {
    ...
    if (!leaderboardData || leaderboardData.length === 0) {
        grid.innerHTML = `
            <div style="text-align: center; padding: 5rem; color: #64756A; font-size: 1.4rem;">
    ...
    grid.innerHTML = leaderboardData.map((t, idx) => {
        ...
        const colorGradient = teamColorMap[t.name] || 'linear-gradient(90deg, #F59E0B, #F59E0B)';
...
function renderCategoryLeaders(catName) {
    ...
    if (firstPlace) {
        const firstColor = teamColorMap[firstPlace.name] || 'linear-gradient(90deg, #F59E0B, #F59E0B)';
        html += `
        ...
        if (secondPlace) {
            html += `
                <div class="cat-runner-card">
                    ...
                    <div class="cat-runner-points">${secondPlace.points} <span style="font-size:1rem; color:#64756A;">pts</span></div>
                </div>
            `;
        }
        ...
        if (thirdPlace) {
            html += `
                <div class="cat-runner-card">
                    ...
                    <div class="cat-runner-points">${thirdPlace.points} <span style="font-size:1rem; color:#64756A;">pts</span></div>
                </div>
            `;
        }
    ...
    container.innerHTML = catData.teams.map((t) => {
        ...
        return `
            <div class="cat-comp-card">
                ...
                <div class="cat-comp-points-num">${t.points} <span style="font-size:1.1rem; color:#64756A;">pts</span></div>
            </div>
        `;
    }).join('');
}
...
async function initLiveDisplayEngine() {
    if (!instId) {
        ...
        if (grid) {
            grid.innerHTML = `
                <div style="text-align: center; padding: 4rem 2rem; color: #D8E8DE;">
                    <div style="font-size: 3rem; margin-bottom: 1rem;">🏆</div>
                    <h2 style="font-size: 1.5rem; font-weight: 800; color: #F59E0B; margin-bottom: 0.5rem;">No Championship Selected</h2>
                    <p style="font-size: 1rem; color: #64756A;">Please open Live Display from the Admin Dashboard.</p>
                </div>
            `;
        }
        return;
