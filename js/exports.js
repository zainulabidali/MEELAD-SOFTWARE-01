...
async function compilePDF(exp, f, programs, resultsList, participantsMap, studentMap = {}, isDownload = false, classAwards = []) {
    ...
    if (f.type === 'Green Room Sign') {
        try {
            const { generateGreenRoomHTML } = await import('./exports/features/greenroom.js');
            htmlContent += generateGreenRoomHTML(f, programs, participantsMap, isCompact, formatLabel);
        } catch (err) {
            console.error("Failed to dynamically load Green Room module:", err);
            window.showToast("Failed to load Green Room module.", "error");
        }
    }
...
async function compileCSV(exp, f, programs, resultsList, participantsMap, studentMap = {}, classAwards = []) {
    ...
    if (f.type === 'Green Room Sign') {
        try {
            const { generateGreenRoomCSV } = await import('./exports/features/greenroom.js');
            csvContent += generateGreenRoomCSV(f, programs, participantsMap);
        } catch (err) {
            console.error("Failed to dynamically load Green Room CSV module:", err);
            window.showToast("Failed to load Green Room module.", "error");
        }
    }
