    ...
    if (!isAllowed) {
        container.innerHTML = `
            <div class="card" style="padding: 2rem; text-align: center; max-width: 500px; margin: 2rem auto;">
                ...
                <p style="color:#64756A; margin-bottom: 1rem;">This page is available ONLY for Admin.</p>
    ...
    container.innerHTML = `
        <div class="finance-container" style="display:flex; flex-direction:column; gap:1.25rem;">
            ...
            <div class="summary-cards-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:1rem;">
                ...
                <!-- Total Income -->
                <div class="card" style="padding:1.25rem; background:linear-gradient(135deg, #ECFDF5, #D1FAE5); border:1px solid #bbf7d0; border-radius:12px;">
                    <div style="font-size:0.75rem; font-weight:700; color:#065F46; text-transform:uppercase; letter-spacing:0.05em;">Total Income</div>
                    <div id="statTotalIncome" style="font-size:1.6rem; font-weight:900; color:#047857; margin-top:0.35rem;">₹0</div>
                </div>
                ...
                <div class="card" style="padding:1.25rem; background:linear-gradient(135deg, #fef2f2, #fee2e2); border:1px solid #fecaca; border-radius:12px;">
                    ...
                    <div id="statTotalExpense" style="font-size:1.6rem; font-weight:900; color:#EF4444; margin-top:0.35rem;">₹0</div>
                </div>
                ...
                <!-- Income Entries -->
                <div class="card" style="padding:1.25rem; background:#ffffff; border:1px solid #D8E8DE; border-radius:12px;">
                    <div style="font-size:0.75rem; font-weight:700; color:#64756A; text-transform:uppercase; letter-spacing:0.05em;">Income Entries</div>
                    <div id="statIncomeCount" style="font-size:1.6rem; font-weight:900; color:#17251B; margin-top:0.35rem;">0</div>
                </div>

                <!-- Expense Entries -->
                <div class="card" style="padding:1.25rem; background:#ffffff; border:1px solid #D8E8DE; border-radius:12px;">
                    <div style="font-size:0.75rem; font-weight:700; color:#64756A; text-transform:uppercase; letter-spacing:0.05em;">Expense Entries</div>
                    <div id="statExpenseCount" style="font-size:1.6rem; font-weight:900; color:#17251B; margin-top:0.35rem;">0</div>
                </div>
            </div>
            ...
            <div class="card" style="padding:1rem 1.25rem; display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:0.75rem; border-radius:12px;">
                <div style="display:flex; gap:0.75rem; flex-wrap:wrap;">
                    <button type="button" id="btnOpenAddIncome" class="btn" style="background:#059669; color:#ffffff; font-weight:800; padding:0.6rem 1.25rem; border:none; border-radius:8px; cursor:pointer;">
                        + Add Income
                    </button>
                    <button type="button" id="btnOpenAddExpense" class="btn" style="background:#EF4444; color:#ffffff; font-weight:800; padding:0.6rem 1.25rem; border:none; border-radius:8px; cursor:pointer;">
                        + Add Expense
                    </button>
                </div>
            ...
            <div style="display:grid; grid-template-columns: 2fr 1fr; gap:1.25rem;" class="finance-content-grid">
                ...
                <div class="card" style="padding:1.25rem; border-radius:12px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #ECFDF5; padding-bottom:0.75rem; margin-bottom:1rem;">
                        <h3 style="margin:0; font-size:1.1rem; font-weight:800; color:#17251B;">Recent Records</h3>
                        <span style="font-size:0.8rem; color:#64756A;" id="recordCountTag">0 Records</span>
                    </div>
                    <div id="recentRecordsContainer" style="overflow-x:auto;">
                        <div style="text-align:center; padding:2rem; color:#64756A;">Loading records...</div>
                    </div>
                </div>

                <!-- Right: Expense Summary List -->
                <div class="card" style="padding:1.25rem; border-radius:12px;">
                    <div style="border-bottom:1px solid #ECFDF5; padding-bottom:0.75rem; margin-bottom:1rem;">
                        <h3 style="margin:0; font-size:1.1rem; font-weight:800; color:#17251B;">Expense Summary</h3>
                        <p style="margin:0.25rem 0 0 0; font-size:0.75rem; color:#64756A;">Total expense by category</p>
                    </div>
                    <div id="expenseSummaryContainer">
                        <div style="text-align:center; padding:2rem; color:#64756A;">No expense data</div>
                    </div>
                </div>
...
function renderRecentRecordsTable() {
    ...
    if (localRecords.length === 0) {
        container.innerHTML = `
            <div style="text-align:center; padding:3rem 1rem; color:#64756A;">
                <div style="font-size:2rem; margin-bottom:0.5rem;">📝</div>
                <div style="font-weight:700; color:#64756A;">No records added yet</div>
    ...
    let html = `
        <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.88rem;">
            <thead>
                <tr style="background:#F5FAF7; border-bottom:2px solid #D8E8DE; color:#64756A;">
    ...
    localRecords.forEach((item) => {
        ...
        const typeBadge = isIncome
            ? `<span style="background:#D1FAE5; color:#047857; font-weight:800; font-size:0.75rem; padding:3px 8px; border-radius:6px; display:inline-block;">Income</span>`
        ...
        const amtColor = isIncome ? '#059669' : '#EF4444';

        html += `
            <tr style="border-bottom:1px solid #ECFDF5;">
                <td style="padding:0.75rem; font-weight:600; color:#64756A; white-space:nowrap;">${safeEscape(item.date || '')}</td>
                <td style="padding:0.75rem;">${typeBadge}</td>
                <td style="padding:0.75rem; font-weight:700; color:#17251B;">${safeEscape(item.category || item.source || '-')}</td>
                <td style="padding:0.75rem; text-align:right; font-weight:800; color:${amtColor}; white-space:nowrap;">${formattedAmt}</td>
                <td style="padding:0.75rem; color:#64756A; font-size:0.82rem;">${safeEscape(item.remarks || '-')}</td>
                <td style="padding:0.75rem; text-align:center; white-space:nowrap;">
                    ...
                    <button type="button" class="btn-del-rec" data-id="${item.id}" style="background:none; border:none; color:#EF4444; font-weight:700; cursor:pointer; font-size:0.82rem;">Delete</button>
                </td>
            </tr>
        `;
    });
...
function renderExpenseSummaryList(categoryMap, totalExpense) {
    ...
    categories.forEach(cat => {
        ...
        html += `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:0.6rem 0.75rem; background:#F5FAF7; border-radius:8px; border:1px solid #D8E8DE;">
                <span style="font-weight:700; color:#64756A; font-size:0.88rem;">${cat}</span>
                <span style="font-weight:800; color:${val > 0 ? '#EF4444' : '#64756A'}; font-size:0.9rem;">${formattedVal}</span>
            </div>
        `;
    });
...
function openAddModal(type) {
    ...
    const modalBody = `
        <form id="formFinanceRecord" style="display:flex; flex-direction:column; gap:1rem;">
            <div>
                <label style="display:block; font-size:0.82rem; font-weight:700; color:#64756A; margin-bottom:0.3rem;">Date *</label>
                <input type="date" id="finDate" required value="${today}" class="form-input-compact" style="width:100%; box-sizing:border-box;" />
            </div>

            <div>
                <label style="display:block; font-size:0.82rem; font-weight:700; color:#64756A; margin-bottom:0.3rem;">Amount (₹) *</label>
            ...
            <div>
                <label style="display:block; font-size:0.82rem; font-weight:700; color:#64756A; margin-bottom:0.3rem;">${labelCategory} *</label>
            ...
            <div>
                <label style="display:block; font-size:0.82rem; font-weight:700; color:#64756A; margin-bottom:0.3rem;">Remarks</label>
                <input type="text" id="finRemarks" placeholder="Optional notes" class="form-input-compact" style="width:100%; box-sizing:border-box;" />
            </div>

            <div style="display:flex; gap:0.75rem; justify-content:flex-end; margin-top:0.5rem; border-top:1px solid #D8E8DE; padding-top:1rem;">
                <button type="button" id="btnCancelFinModal" class="btn btn-secondary btn-sm" style="font-weight:700;">Cancel</button>
                <button type="submit" class="btn btn-primary btn-sm" style="font-weight:800; background:${isIncome ? '#059669' : '#EF4444'}; border:none;">Save</button>
            </div>
        </form>
    `;
...
function printReport() {
    ...
    localRecords.forEach(r => {
        rowsHtml += `
            <tr>
                <td style="padding:6px 10px; border:1px solid #D8E8DE;">${r.date || ''}</td>
                <td style="padding:6px 10px; border:1px solid #D8E8DE; font-weight:bold; color:${r.type === 'income' ? '#047857' : '#b91c1c'};">${r.type === 'income' ? 'Income' : 'Expense'}</td>
                <td style="padding:6px 10px; border:1px solid #D8E8DE;">${r.category || r.source || ''}</td>
                <td style="padding:6px 10px; border:1px solid #D8E8DE; text-align:right; font-weight:bold;">₹${(Number(r.amount) || 0).toLocaleString('en-IN')}</td>
                <td style="padding:6px 10px; border:1px solid #D8E8DE;">${r.remarks || ''}</td>
            </tr>
        `;
    });
    ...
    printWin.document.write(`
        ...
        <head>
            ...
            <style>
                body { font-family: Arial, sans-serif; padding: 20px; color: #17251B; }
                h1 { margin-bottom: 4px; font-size: 20px; }
                .subtitle { font-size: 13px; color: #64756A; margin-bottom: 20px; }
                .cards { display: flex; gap: 15px; margin-bottom: 20px; }
                .card { flex: 1; padding: 12px; border: 1px solid #D8E8DE; border-radius: 6px; }
                .card-title { font-size: 11px; color: #64756A; text-transform: uppercase; }
                .card-val { font-size: 18px; font-weight: bold; margin-top: 4px; }
                table { width: 100%; border-collapse: collapse; font-size: 12px; margin-top: 10px; }
                th { background: #ECFDF5; padding: 8px 10px; border: 1px solid #D8E8DE; text-align: left; }
                @media print {
                    body { padding: 0; }
                }
            </style>
        ...
        <body>
            ...
            <div class="cards">
                <div class="card">
                    ...
                    <div class="card-val" style="color:#047857;">₹${totalIncome.toLocaleString('en-IN')}</div>
                </div>
                <div class="card">
                    <div class="card-title">Total Expense</div>
                    <div class="card-val" style="color:#EF4444;">₹${totalExpense.toLocaleString('en-IN')}</div>
                </div>
                <div class="card">
                    <div class="card-title">Balance</div>
                    <div class="card-val" style="color:${balance < 0 ? '#EF4444' : '#1d4ed8'};">₹${balance.toLocaleString('en-IN')}</div>
                </div>
            </div>
