...
function renderStudentsUI() {
    ...
    localStudents.forEach((stu, idx) => {
        ...
        if (!rowDiv) {
            ...
            rowDiv.innerHTML = `
                <div class="student-sl-cell" style="font-weight: 700; color: #64756A;">
                    ${idx + 1}
                </div>
                ...
                <div class="student-programs-cell">
                    <span class="student-programs-badge btn-view-progs-direct" data-id="${id}" style="cursor:pointer; background:rgba(4, 120, 87, 0.08); color:#065F46; border:1px solid rgba(4, 120, 87, 0.15); display:inline-flex; align-items:center; gap:0.25rem;" title="Click to view registered programs">
                        👥 View Programs
                    </span>
                </div>
...
async function openViewProgramsModal(stu) {
    ...
    modalBody.innerHTML = `
        <div style="text-align:center; padding:3rem; color:#065F46;">
            <div class="spinner" style="margin:0 auto 1rem; width:40px; height:40px; border:4px solid rgba(4, 120, 87,0.1); border-top-color:#065F46; border-radius:50%; animation:spin 1s linear infinite;"></div>
            <p style="color:#64756A; font-size:0.875rem; font-weight:600;">Retrieving registered programs...</p>
        </div>
        <div class="modal-actions" style="margin-top:1.5rem; border-top:1px solid #D8E8DE; padding-top:1rem; display:none;">
    ...
    if (uniqueProgs.length === 0) {
        studentProgsHTML = `
            <div style="text-align:center; padding:2rem; background:#F5FAF7; border:1px dashed #D8E8DE; border-radius:8px; color:#64756A; font-size:0.85rem; font-style:italic;">
        ...
        studentProgsHTML = `
            <div style="display:flex; flex-direction:column; gap:0.6rem; max-height:260px; overflow-y:auto; padding-right:4px;">
                ${uniqueProgs.map((p) => {
            return `
                        <div style="background:#ffffff; border:1px solid #D8E8DE; border-radius:8px; padding:0.8rem 1rem; box-shadow:0 1px 2px rgba(0,0,0,0.02); display:flex; flex-direction:column; gap:0.4rem;">
                            <div style="font-size:0.92rem; font-weight:700; color:#17251B;">
                                ${window.escapeHTML(p.name)}
                            </div>
                            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:0.3rem 0.75rem; font-size:0.78rem; color:#64756A;">
                                <div><span style="font-weight:600; color:#64756A;">Type:</span> ${window.escapeHTML(p.type)}</div>
                                <div><span style="font-weight:600; color:#64756A;">Mode:</span> ${window.escapeHTML(p.regMode)}</div>
                                <div><span style="font-weight:600; color:#64756A;">Location:</span> ${window.escapeHTML(p.location)}</div>
                                <div><span style="font-weight:600; color:#64756A;">Category:</span> ${window.escapeHTML(p.category)}</div>
                                ${p.teamName ? `<div style="grid-column: span 2;"><span style="font-weight:600; color:#64756A;">Team:</span> ${window.escapeHTML(p.teamName)}</div>` : ''}
                            </div>
                        </div>
                    `;
        }).join('')}
    ...
    modalBody.innerHTML = `
        <div style="font-family:'Inter',sans-serif; color:#17251B;">
            <div style="background:#F5FAF7; border:1px solid #D8E8DE; border-radius:12px; padding:1rem; margin-bottom:1.25rem; display:flex; flex-direction:column; gap:0.5rem;">
                <div style="font-size:0.72rem; font-weight:700; color:#64756A; text-transform:uppercase; letter-spacing:0.05em;">Student Profile</div>
                <div style="font-size:1.05rem; font-weight:800; color:#17251B; display:flex; align-items:center; gap:0.5rem;">
                    👤 ${window.escapeHTML(stu.name)}
                    <span style="background:#e0e7ff; color:#064E3B; border-radius:999px; padding:0.15rem 0.6rem; font-size:0.7rem; font-weight:700;">#${window.escapeHTML(stu.chestNumber || '')}</span>
                </div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:0.5rem 1rem; margin-top:0.25rem; font-size:0.85rem;">
                    <div><span style="color:#64756A; font-weight:600;">Gender:</span> ${window.escapeHTML(stu.gender || '—')}</div>
                    <div><span style="color:#64756A; font-weight:600;">Team:</span> ${window.escapeHTML(teamName)}</div>
                    <div><span style="color:#64756A; font-weight:600;">Category:</span> ${window.escapeHTML(stu.categoryName || 'General')}</div>
                    <div><span style="color:#64756A; font-weight:600;">Class:</span> ${window.escapeHTML(stu.className || 'Standard')}</div>
                </div>
            </div>

            <div style="margin-bottom:0.5rem;">
                <div style="font-size:0.72rem; font-weight:700; color:#64756A; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:0.75rem;">REGISTERED PROGRAMS (${uniqueProgs.length})</div>
                ${studentProgsHTML}
            </div>

            <div class="modal-actions" style="margin-top:1.5rem; border-top:1px solid #D8E8DE; padding-top:1rem;">
                <button class="btn btn-secondary w-full" id="closeViewModalBtn" style="min-height:38px; font-weight:700;">Close</button>
            </div>
        </div>
    `;
...
export async function initAddStudentView(container, topActions) {
    ...
    container.innerHTML = `
        <div style="text-align:center; padding:3rem; color:#065F46;">
            <div class="spinner" style="margin:0 auto 1rem; width:40px; height:40px; border:4px solid rgba(4, 120, 87,0.1); border-top-color:#065F46; border-radius:50%; animation:spin 1s linear infinite;"></div>
            <p style="color:#64756A; font-size:0.875rem; font-weight:600;">Loading configurations...</p>
        </div>
    `;
    ...
    container.innerHTML = `
        ...
        <div class="premium-container">
            <div class="premium-card">
                <div style="border-bottom: 1px solid #D8E8DE; padding-bottom: 1rem; margin-bottom: 0.5rem; display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <h2 style="margin:0; font-size:1.4rem; font-weight:800; color:#17251B;">Student Registration</h2>
                        <p style="margin:0.25rem 0 0 0; font-size:0.875rem; color:#64756A;">Enroll multiple students at once into a specific category, team, gender, and class.</p>
                    </div>
                ...
                <div class="flow-grid">
                    ...
                    <div class="form-group" style="margin:0;">
                        <label class="form-label" style="display:flex; align-items:center; gap:0.5rem; cursor:pointer; font-weight:700; color:#64756A; user-select:none;">
                ...
                <div id="chestPreviewPanel" class="preview-panel" style="display: none;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem 1rem;">
                        <div>Highest Existing: <strong id="lblHighestChest" style="color: #17251B;">—</strong></div>
                        <div>Next Available: <strong id="lblNextChest" style="color: #065F46;">—</strong></div>
                        <div style="grid-column: span 2; border-top: 1px solid #D8E8DE; margin-top: 0.25rem; padding-top: 0.25rem;">
                            Expected Range for Batch: <strong id="lblExpectedRange" style="color: #17251B;">—</strong>
                        </div>
                    </div>
                </div>
                ...
                <!-- Action Buttons -->
                <div style="display: flex; gap: 0.75rem; justify-content: flex-end; border-top: 1px solid #D8E8DE; padding-top: 1.25rem;">
    ...
    function calculateChestPreviewAndStats() {
        ...
        if (firstAvailable) {
            ...
            lblNextChest.style.color = '#065F46';
        } else {
        ...
        if (studentsToAllocateCount === 0) {
            ...
            lblExpectedRange.style.color = '#64756A';
            ...
            lblExpectedRange.style.color = '#047857';
        }
    ...
    modalBody.innerHTML = `
        <form id="editStudentForm">
            <div class="form-group">
                ...
                <input type="text" id="eChest" class="form-input" disabled readonly style="background:#ECFDF5; cursor:not-allowed; font-weight:700; color:#64756A;" value="#${window.escapeHTML(data.chestNumber || '—')}">
                <p style="font-size:0.72rem; color:#64756A; margin-top:0.25rem;">Chest number is allocated automatically and cannot be changed.</p>
            </div>
            <div class="form-group">
            ...
            <div class="form-group" style="display:flex; align-items:center; gap:0.5rem; margin-top:1rem;">
                <label class="form-label" style="margin:0; display:flex; align-items:center; gap:0.5rem; cursor:pointer; font-weight:700; color:#64756A; user-select:none;">
...
async function openBulkImportModal() {
    ...
    modalBody.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:1.25rem; padding:0.25rem;">
            <p style="font-size:0.85rem; color:#64756A; margin:0; line-height:1.5;">
            ...
            <div style="display:flex; flex-direction:column; gap:0.75rem; background:#F5FAF7; border:1px solid #D8E8DE; border-radius:10px; padding:1rem;">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.5rem;">
                    <span style="font-size:0.8rem; font-weight:700; color:#64756A;">1. Need a template?</span>
                    <button class="btn btn-secondary btn-sm" id="btnDownloadTemplate" style="font-size:0.75rem;">📥 Download Sample Template</button>
                </div>
                <div style="border-top:1px solid #D8E8DE; padding-top:0.75rem; margin-top:0.25rem; display:flex; flex-direction:column; gap:0.5rem;">
                    <label style="font-size:0.8rem; font-weight:700; color:#64756A;">2. Select Excel File (.xlsx, .xls) *</label>
                    <input type="file" id="excelFileInput" accept=".xlsx, .xls" class="form-input" style="padding:0.4rem;" />
                    <span style="font-size:0.7rem; color:#64756A;">Maximum file size: 10 MB.</span>
                </div>
            </div>
    ...
    modalBody.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:1rem; padding:0.25rem;">
            ...
            <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:0.4rem; text-align:center;">
                <div style="background:#F5FAF7; border:1px solid #D8E8DE; border-radius:8px; padding:0.5rem 0.25rem;">
                    <div style="font-size:1.1rem; font-weight:800; color:#64756A;">${totalRows}</div>
                    <div style="font-size:0.6rem; font-weight:700; color:#64756A; text-transform:uppercase; margin-top:0.15rem;">Total Rows</div>
                </div>
                <div style="background:#ECFDF5; border:1px solid #bbf7d0; border-radius:8px; padding:0.5rem 0.25rem;">
                    <div style="font-size:1.1rem; font-weight:800; color:#065F46;">${validList.length}</div>
                    <div style="font-size:0.6rem; font-weight:700; color:#047857; text-transform:uppercase; margin-top:0.15rem;">To Import</div>
                </div>
            ...
            <div>
                <div style="font-size:0.75rem; font-weight:700; color:#64756A; text-transform:uppercase; margin-bottom:0.4rem;">Students to Enroll (${validList.length})</div>
                <div style="max-height:160px; overflow-y:auto; border:1px solid #D8E8DE; border-radius:8px; padding:0.25rem; background:#fff; font-size:0.8rem;">
                    ${validList.length > 0
            ? `<table style="width:100%; border-collapse:collapse;">
                            <thead style="position:sticky; top:0; background:#ECFDF5; font-weight:700; z-index:10;">
                                <tr style="border-bottom:1px solid #D8E8DE; text-align:left;">
                            ...
                            <tbody>
                                ${validList.map(item => `
                                    <tr style="border-bottom:1px solid #ECFDF5;">
                                        <td style="padding:0.35rem; font-weight:700; color:#1e1b4b;">${item.chestNumber || '<span style="color:#059669; font-style:italic; font-size:0.75rem;">(Auto)</span>'}</td>
                                        <td style="padding:0.35rem; color:#17251B;">${window.escapeHTML(item.name)}</td>
                                        <td style="padding:0.35rem; color:#64756A;">${window.escapeHTML(item.categoryName)} / ${window.escapeHTML(item.className)}</td>
                                        <td style="padding:0.35rem; color:#64756A;">${window.escapeHTML(item.teamName || '—')}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                           </table>`
            : '<div style="font-size:0.8rem; color:#64756A; text-align:center; padding:1.5rem 0;">No clean rows to import</div>'
        }
                </div>
            </div>
            ...
            <!-- Warning Banner -->
            ${validList.length > 0 ? `
            <div style="font-size:0.72rem; color:#64756A; background:#eff6ff; border:1px solid #bfdbfe; border-radius:8px; padding:0.6rem 0.85rem; line-height:1.4;">
            ...
            <!-- Action buttons -->
            <div style="display:flex; gap:0.5rem; justify-content:flex-end; border-top:1px solid #D8E8DE; padding-top:1rem; margin-top:0.25rem;">
...
async function executeImportProcess(validList, duplicateList, duplicateAction) {
    ...
    modalBody.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:1.25rem; padding:0.5rem; text-align:center;">
            ...
            <h4 style="margin:0; font-weight:800; font-size:1.05rem; color:#17251B;" id="importProgressTitle">Importing Students...</h4>
            <p style="font-size:0.8rem; color:#64756A; line-height:1.4; margin:0;" id="importProgressDesc">
                Processing records in chunked database batches. Please wait.
            </p>

            <!-- Progress bar -->
            <div style="width:100%; height:12px; background:#D8E8DE; border-radius:99px; overflow:hidden; position:relative; margin:0.5rem 0;">
                <div id="importProgressBar" style="width:0%; height:100%; background:#064E3B; border-radius:99px; transition:width 0.25s;"></div>
            </div>
            <div style="font-size:0.75rem; font-weight:700; color:#064E3B;" id="importProgressPercent">0% Complete</div>
        </div>
    `;
    ...
    if (failedRows.length === 0) {
        modalBody.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:1.25rem; padding:0.5rem; text-align:center;">
                ...
                <h4 style="margin:0; font-weight:800; font-size:1.15rem; color:#047857;">Import Completed Successfully!</h4>
                <p style="font-size:0.85rem; color:#64756A; line-height:1.5; margin:0;">
...
function openStudentDropdown(btn) {
    ...
    dropdown.innerHTML = `
        <button class="dropdown-item btn-view-stu" style="display:flex; align-items:center; gap:0.5rem; width:100%; border:none; background:transparent; padding:0.5rem 0.85rem; font-size:12px; font-weight:600; color:#64756A; text-align:left; cursor:pointer;">
            👁️ View Programs
        </button>
        <button class="dropdown-item btn-edit-stu" style="display:flex; align-items:center; gap:0.5rem; width:100%; border:none; background:transparent; padding:0.5rem 0.85rem; font-size:12px; font-weight:600; color:#64756A; text-align:left; cursor:pointer;">
            ✏️ Edit Student
        </button>
        <button class="dropdown-item btn-delete-stu text-danger" style="display:flex; align-items:center; gap:0.5rem; width:100%; border:none; background:transparent; padding:0.5rem 0.85rem; font-size:12px; font-weight:600; color:#EF4444; text-align:left; cursor:pointer;">
            🗑️ Delete Student
        </button>
    `;
