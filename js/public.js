...
function buildResultCard(result) {
    ...
    card.innerHTML = `
        ...
        <div class="result-card-body">
            ${winnersHTML || '<p style="color:#64756A; font-size:0.875rem; text-align:center; padding:1rem;">No winners recorded.</p>'}
        </div>
...
function downloadResultImage(result) {
    ...
    const winnersHTML = winners.map(w => {
        ...
        return `
            <div style="display:flex; align-items:center; gap:12px; padding:10px 14px; margin-bottom:8px; border-radius:10px; background:#F5FAF7; border-left:4px solid #064E3B;">
                <span style="font-size:24px;">${medal}</span>
                <div>
                    <div style="font-weight:700; color:#17251B; font-size:14px;">${w.studentName}</div>
                    <div style="font-size:11px; color:#64756A; margin-top:2px;">${w.position} Place${gradeStr}${marksStr}</div>
                </div>
            </div>
        `;
    }).join('');
    ...
    target.innerHTML = `
        <!-- Header -->
        <div style="background: linear-gradient(135deg, #1e1b4b, #064E3B); padding: 28px 32px; color: white;">
        ...
        <div style="padding: 24px 32px;">
            <div style="font-size:11px; font-weight:700; color:#64756A; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:14px;">🏆 RESULTS</div>
            ${winnersHTML}
        </div>
        <!-- Footer -->
        <div style="padding: 14px 32px 20px; border-top: 1px solid #D8E8DE; display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:10px; color:#64756A;">Institute Event Portal</span>
            <span style="font-size:10px; color:#64756A;">Published: ${printDate}</span>
        </div>
    `;
    ...
    html2canvas(target, {
        ...
        document.body.removeChild(target);
        window.customAlert('Image download failed. Please try again.', 'Error', { icon: '⚠️', iconColor: '#EF4444', iconBg: 'rgba(220, 38, 38, 0.08)' });
    });
}
