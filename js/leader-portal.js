...
function renderDashboard() {
    ...
    if (displayedPrograms.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:3rem; color:#64756A; font-style:italic; font-weight:500; grid-column: 1 / -1;">No programs found matching conditions.</div>`;
        return;
    }
    ...
    displayedPrograms.forEach(p => {
        ...
        card.innerHTML = `
            <div class="program-info">
                ...
                <strong style="font-size: 1.05rem; color: #17251B;">${p.programNumber ? `[#${p.programNumber}] ` : ''}${window.escapeHTML(p.programName)}</strong>
...
function renderStudentsCheklist() {
    ...
    if (filtered.length === 0) {
        listEl.innerHTML = `<div class="pw-empty" style="padding:2rem; text-align:center; color:#64756A; font-size:0.8rem;">No eligible students found matching conditions.</div>`;
        return;
    }
...
function renderSelectionBufferAndSummary() {
    ...
    if (selectedCount === 0) {
        ...
        previewContainer.innerHTML = `<div style="padding:1.5rem; text-align:center; color:#64756A; font-size:0.8rem; font-style:italic;">No students currently selected in checklist.</div>`;
    } else {
        ...
        const chips = [...selectedStudentIds].map(id => {
            ...
            const secondaryText = details.length > 0 ? ` <small style="color: #64756A; font-weight: normal; margin-left: 4px;">(${window.escapeHTML(details.join(', '))})</small>` : '';
            return `
                <div class="pw-chip" style="margin: 0.15rem; display: inline-flex; align-items: center; background: #ffffff; border: 1.5px solid #D8E8DE; padding: 0.35rem 0.65rem; border-radius: 20px; font-size: 0.78rem; color: #64756A; font-weight: 600; gap: 0.4rem; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.03); transition: all 0.2s ease;">
                    <span>${window.escapeHTML(s.name)}${secondaryText}</span>
                    <button class="pw-chip-remove" data-id="${s.id}" style="background: transparent; border: none; color: #64756A; font-size: 1rem; line-height: 1; cursor: pointer; padding: 0; display: inline-flex; align-items: center; justify-content: center; width: 16px; height: 16px; border-radius: 50%; transition: all 0.15s ease;">&times;</button>
                </div>
            `;
        }).join('');
    ...
    if (isGroupEvent) {
        ...
        footer.innerHTML = `
            <div style="width:100%; display:flex; flex-direction:column; gap:0.5rem;">
                ...
                <div id="pwSaveStatus" style="font-size:0.72rem; color:#64756A; text-align:center;"></div>
            </div>
        `;
...
function renderAssignedRightPanel() {
    ...
    if (isGroupEvent) {
        ...
        if (teamGroups.length === 0) {
            container.innerHTML = `<div class="pw-empty" style="padding:2rem; text-align:center; color:#64756A; font-size:0.8rem; border:1.5px dashed #D8E8DE; border-radius:8px;">No groups created.</div>`;
            return;
        }

        container.innerHTML = teamGroups.map(g => {
            const memberNames = (g.members || []).map(m => `
                <div style="display:flex; justify-content:space-between; align-items:center; background:#ECFDF5; padding:0.4rem 0.6rem; border-radius:6px; font-size:0.75rem; color:#64756A; font-weight:600;">
            ...
            return `
                <div class="pw-group-card" style="border:1.5px solid var(--pw-border); border-radius:10px; padding:1rem; margin-bottom:0.75rem; background:white;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem;">
                        <h4 style="margin:0; font-size:0.9rem; font-weight:800; color:#17251B;">${window.escapeHTML(g.name)}</h4>
                        <button class="btn btn-secondary btn-sm btn-delete-group text-danger" data-id="${g.id}" style="padding:3px 8px; font-size:0.7rem; font-weight:700; color:#EF4444; border-color:rgba(220,38,38,0.2);" ${resultSubmitted || resultPublished ? 'disabled' : ''}>🗑️ Delete Group</button>
                    </div>
        ...
        if (assignedParticipantsAll.length === 0) {
            container.innerHTML = `<div class="pw-empty" style="padding:2rem; text-align:center; color:#64756A; font-size:0.8rem; border:1.5px dashed #D8E8DE; border-radius:8px;">No participants assigned.</div>`;
            return;
        }
            ...
            <tr>
                ...
                <td>
                    <button class="btn-action-icon pw-part-delete-btn text-danger" data-id="${p.studentId}" ${resultSubmitted || resultPublished ? 'disabled style="opacity:0.5; cursor:not-allowed;"' : ''} title="Delete">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" style="width:0.85rem; height:0.85rem; color:#EF4444;">
...
function openParticipantDropdownMenu(btn) {
    ...
    dropdown.innerHTML = `
        <button class="dropdown-item pw-dropdown-edit-btn" data-id="${id}" style="display:flex; align-items:center; gap:0.5rem; width:100%; border:none; background:transparent; padding:0.5rem 0.85rem; font-size:12px; font-weight:600; color:#64756A; text-align:left; cursor:pointer;">✏️ Edit</button>
        <button class="dropdown-item btn-danger-item pw-dropdown-delete-btn text-danger" data-id="${id}" style="display:flex; align-items:center; gap:0.5rem; width:100%; border:none; background:transparent; padding:0.5rem 0.85rem; font-size:12px; font-weight:600; color:#EF4444; text-align:left; cursor:pointer;">🗑️ Delete</button>
    `;
